...
namespace Lidgren.Network {
    public partial class NetPeer {
        ...
        private Mutex CreateMutex (bool initallyOwned, string name) {
            try {
                return new Mutex(initallyOwned, name);
            } catch (NotSupportedException) {
                return new Mutex(initallyOwned);
            }
        }
        ...
        private void BindSocket (bool reBind) {
            ...
            using (var mutex = CreateMutex(false, "Global\\lidgrenSocketBind")) {
                try {
                    mutex.WaitOne();
        ...
        private void Heartbeat () {
            ...
            if (!m_socket.Poll(m_configuration.m_networkLoopIntervalMicroSecond, SelectMode.SelectRead)) // wait up to 1 ms for data to arrive
                return;
